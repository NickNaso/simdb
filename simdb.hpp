...
public:
  ...
  static constexpr u32        LIST_END = 0xFFFFFFFF;
  static constexpr u32 NXT_VER_SPECIAL = 0xFFFFFFFF;
...
public:
  ...
  static constexpr u32 LIST_END = CncrLst::LIST_END;
...
public:
  ...
  static constexpr u32  KEY_MAX          =   0xFFFFFFFF; 
  static constexpr u32  EMPTY            =   KEY_MAX;              // first 21 bits set 
  static constexpr u32  DELETED          =   KEY_MAX - 1;          // 0xFFFFFFFE;       // 1 less than the EMPTY
  static constexpr u32  LIST_END         =   CncrStr::LIST_END;
  static constexpr u32  SLOT_END         =   CncrStr::LIST_END;
...
private:
  ...
  auto       runIfMatch(VerIdx vi, const void* const buf, u32 len, u32 hash, FUNC f, T defaultRet = decltype(f(VerIdx{}))() ) const -> std::pair<Match, T>   // std::pair<Match, decltype(f(vi))>
  { 
    Match m;
...
public:
  static constexpr u32        EMPTY = CncrHsh::EMPTY;              // 28 bits set 
  static constexpr u32      DELETED = CncrHsh::DELETED;            // 28 bits set 
  static constexpr u32   FAILED_PUT = CncrHsh::EMPTY;              // 28 bits set 
  static constexpr u32     SLOT_END = CncrHsh::SLOT_END;
  static constexpr u32     LIST_END = CncrStr::LIST_END;
...
public:
  ...
  [[nodiscard]] u32          cur() const { return m_curChIdx; }
  [[nodiscard]] auto        data() const -> const void* const { return s_cs.data(); }                   // return a pointer to the start of the block data
  [[nodiscard]] u64         size() const { return CncrStr::sizeBytes( (u32)s_blockSize->load(), (u32)s_blockCount->load()); }
  [[nodiscard]] bool     isOwner() const { return m_mem.owner; }
  [[nodiscard]] u64       blocks() const { return s_blockCount->load(); }                               // return the total number of blocks the shared memory
  [[nodiscard]] u64    blockSize() const { return s_blockSize->load();  }
  [[nodiscard]] auto         mem() const -> void* { return m_mem.hndlPtr; }                             // returns a pointer to the start of the shared memory, which will contain the data structures first
  [[nodiscard]] u64      memsize() const { return m_mem.size; }
  [[nodiscard]] auto    hashData() const -> void const* const { return s_ch.data(); }
  bool       close()
  {
    if(m_isOpen){
      m_isOpen = false;
  ...
  [[nodiscard]] auto       error() const -> simdb_error
  {
    return m_error;
  }
